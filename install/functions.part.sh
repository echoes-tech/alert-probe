get() {
	...
	then
		...
		if [ -x '/usr/bin/curl' ] 
	  	...
	  	then	
	  		printf "GET %s?%s HTTP/1.1\r\nConnection: close\r\n\r\n" "$1" "$2" | netcat ${API_HOST} ${API_PORT} > ${3}_tmp 2> /dev/null
			sed '/^\r*$/q' ${3}_tmp > ${3}_header
			sed '1,/^\r*$/d' ${3}_tmp > ${3}
	  	else
	    		clean_and_exit "can't find HTTP command line client (curl or telnet)."
	  	fi	
	fi 
...
post() {
	...
	then
		...
		if [ -x '/usr/bin/curl' ] 
	  	...
	  	then	
	  		printf "POST %s?%s HTTP/1.1\r\nContent-Type: application/json; charset=utf-8\r\nContent-length: $(echo $3 | wc -m)\r\nConnection: close\r\n\r\n%s\r\n" "$1" "$2" "$3" | netcat ${API_HOST} ${API_PORT} > ${4}_tmp 2> /dev/null
	    		sed '/^\r*$/q' ${4}_tmp > ${4}_header
	    		sed '1,/^\r*$/d' ${4}_tmp > ${4}
	  	else
	    		clean_and_exit "can't find HTTP command line client (curl or telnet)."
